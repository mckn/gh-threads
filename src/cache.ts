...
export class TeamCacheManager {
  ...
  async getCachedTeams(username: string): Promise<GitHubTeam[] | null> {
    try {
      ...
      if (cacheAge > this.cacheTTL) {
        this.logger.debug(
          `Cache for user ${username} is expired (age: ${Math.round(
            cacheAge / (60 * 60 * 1000)
          )} hours)`
        );
        return null;
      ...
      if (cacheData.username !== username) {
        this.logger.debug(
          `Cache file is for different user (${cacheData.username}), ignoring`
        );
        return null;
      }

      this.logger.debug(
        `Using cached teams for user ${username} (age: ${Math.round(
          cacheAge / (60 * 60 * 1000)
        )} hours)`
      );
  ...
  async saveTeamsToCache(username: string, teams: GitHubTeam[]): Promise<void> {
    try {
      ...
      this.logger.debug(
        `Saved ${teams.length} teams to cache for user ${username}`
      );
    } catch (error) {
  ...
  async invalidateCache(username: string): Promise<void> {
    try {
      ...
      if (await fs.pathExists(cacheFilePath)) {
        ...
        this.logger.debug(
          `No cache file found to invalidate for user ${username}`
        );
      }
    } catch (error) {
      this.logger.warn(
